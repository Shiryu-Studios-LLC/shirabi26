...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Manage calendar events: list events in a date range, create, update, delete. Each event can carry a tag/category (event_type) and importance level. Resolve relative dates like today/tomorrow against the 'Current date and time' system context, then pass ISO 8601 datetimes in the user's local wall time; for all-day events set all_day=true and pass YYYY-MM-DD. For event reminders/alarms, pass reminder_minutes; the tool creates the Shirabe note reminder, so do not also call manage_notes for the same reminder.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string",
                    ...
                    "reminder_minutes": {"type": "integer", "description": "For create_event: create an Shirabe reminder this many minutes before the event, e.g. 5 for 'reminder 5 min before'."},
...
def function_call_to_tool_block(name: str, arguments: str) -> Optional[ToolBlock]:
    ...
    if tool_type == "bash":
        ...
        if action == "toggle":
            ...
            bg = colors.get("bg", "#1c1c1e")
            fg = colors.get("fg", "#e6d6b8")
            panel = colors.get("panel", "#252528")
            border = colors.get("border", "#443a2e")
            accent = colors.get("accent", "#bd2b32")
