                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Shirabe (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Shirabe" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "shirabe_kind" if "shirabe_kind" in cols else "'scheduled' AS shirabe_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                shirabe_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Shirabe-Origin"] = "shirabe-ui"
                outer["X-Shirabe-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", shirabe_kind or "scheduled")[:64]
                outer["X-Shirabe-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `shirabe-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `SHIRABE_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `shirabe-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("SHIRABE_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `SHIRABE_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (SHIRABE_INPROCESS_POLLERS=0); "
            "drive `shirabe-mail poll-scheduled` externally."
        )
        return
    import asyncio
