# Shirabe Claude Code Integration

This directory contains the Claude Code skill bundle for Shirabe.
...
Claude Code auto-loads anything under `~/.claude/skills/`, so the `shirabe` skill is
available in any session that has `SHIRABE_URL` and `SHIRABE_API_TOKEN` in its
environment.

## What's in the bundle

- `skills/shirabe/SKILL.md` — the skill definition Claude Code reads.
- `skills/shirabe/scripts/shirabe_api.py` — small helper that calls the scoped
...
The token is scope-gated. Every tool surface is checked server-side in Shirabe,
