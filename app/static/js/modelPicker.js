...
const RECENT_KEY = 'shirabe-model-recent';
...
function _initModelPickerDropdown() {
  ...
  const _collapsedProviders = new Set(_loadList('shirabe-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('shirabe-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('shirabe:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('shirabe:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `shirabe-model-favorites[0]`
