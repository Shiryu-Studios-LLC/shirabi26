...
async function _selectAddedModelInChat(endpoint) {
  ...
  try {
    document.dispatchEvent(new CustomEvent('shirabe:auto-select-model', {
      detail: {
...
function initEndpointForm() {
  ...
  document.querySelectorAll('#adm-add-api, #adm-add-local').forEach((sec) => {
    ...
    if (!head) return;
    const key = 'shirabe.addModels.' + sec.id + '.open';
    let open = false;
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const filename = match ? match[1] : 'shirabe_backup.json';
