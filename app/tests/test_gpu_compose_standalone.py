...
base compose with only the matching overlay merged into the ``shirabe``
...
def _merge_overlay_into_base(base: dict, overlay: dict) -> dict:
    """Build the expected standalone config: base + overlay on shirabe only."""
...
# --- Non-shirabe services and volumes untouched ---------------------------


@pytest.mark.parametrize("standalone_path", [NVIDIA_STANDALONE, AMD_STANDALONE])
def test_non_shirabe_services_match_base(base, standalone_path):
...
# --- shirabe = base service + only the overlay additions ------------------


def test_nvidia_shirabe_adds_only_overlay(base):
    standalone = _load(NVIDIA_STANDALONE)
...
def test_amd_shirabe_adds_only_overlay(base):
    standalone = _load(AMD_STANDALONE)
    svc = standalone["services"][SERVICE]
    base_svc = base["services"][SERVICE]
