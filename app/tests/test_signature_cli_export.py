...
def _load_signature_cli(monkeypatch):
    ...
    path = Path(__file__).resolve().parent.parent / "scripts" / "shirabe-signature"
    loader = importlib.machinery.SourceFileLoader("shirabe_signature_cli_under_test", str(path))
